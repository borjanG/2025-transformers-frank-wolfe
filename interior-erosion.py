...
def plot_cells_simple(k=5, i_target=2, eta=1e-3, epsilon=0.02, seed=42):
    ...
    # Step 2: Draw strict dominance cone I(η) \ominus ε B_1 in red
    ...
    if len(pts_cone) > 0:
        try:
            ...
            cone_poly = ShapelyPolygon(hull_cone)
            eroded_cone = cone_poly.buffer(-epsilon)
            if not eroded_cone.is_empty and eroded_cone.geom_type == 'Polygon':
                x, y = eroded_cone.exterior.xy
                plt.fill(x, y, color=hsv_to_rgb([5/6, 1, 1]), edgecolor='black', linewidth=0.5, alpha=0.6)
        except Exception as e:
            print(f"Error in erosion step: {e}")
    ...
    filename = os.path.join(current_dir, f"erosion_k{k}.pdf")
...
if __name__ == "__main__":
    plot_cells_simple(k=7, i_target=2, eta=0.5*1e-1, epsilon=0.03, seed=42)
